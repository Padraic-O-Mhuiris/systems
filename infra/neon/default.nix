{inputs, ...}: {
  perSystem = {pkgs, ...}: let
    ...
    nixos-facter = inputs.nixos-facter-modules.packages.${pkgs.system}.default;

    generateFacterReport = pkgs.writeShellScriptBin "generateFacterReport" ''
      set -euo pipefail
      ...
      HCLOUD_TOKEN=$(${pkgs.pass}/bin/pass show ${vars.PASSWORD_STORE_PATH.HCLOUD_TOKEN})
      export HCLOUD_TOKEN

      SSH_KEY_NAME="neon-primary"
      SSH_PUBLIC_KEY="${inputs.secrets.vars.PRIMARY_USER.SSH_PUBLIC_KEY}"
      ...
      cleanup() {
        if [ -n "''${TEMP_SERVER_NAME:-}" ]; then
          ...
          ${pkgs.hcloud}/bin/hcloud server delete "$TEMP_SERVER_NAME" 2>/dev/null || true
        fi
      }
      ...
      ${pkgs.hcloud}/bin/hcloud ssh-key create --name "$SSH_KEY_NAME" --public-key "$SSH_PUBLIC_KEY" 2>/dev/null || echo "SSH key already exists"

      # Get all SSH key IDs to add to server
      SSH_KEY_IDS=$(${pkgs.hcloud}/bin/hcloud ssh-key list -o noheader -o columns=id | tr '\n' ',' | sed 's/,$//')

      echo "Creating temporary server: $TEMP_SERVER_NAME ($SERVER_TYPE)..."
      SERVER_INFO=$(${pkgs.hcloud}/bin/hcloud server create \
      ...
      SERVER_IP=$(echo "$SERVER_INFO" | ${pkgs.jq}/bin/jq -r '.server.public_net.ipv4.ip')
